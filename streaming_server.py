...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # ── Pipeline Loop Task: token_queue → Bridge → Ditto → send_queue ──────
    gpu_lock = threading.Lock()

    async def pipeline_loop_task():
        """
        The supreme synchronous sequential loop:
        Reads TaggedTokens from Moshi, batches them for Bridge, and sequentially
        executes Ditto completely synchronously inside the GPU lock.
        """
        ...
        async def _flush():
            nonlocal token_buffer, first_token_time, _chunk_id, _flush_reason, seq_counter, frame_count, _drop_count
            if not token_buffer:
                return
            ...
            def _gpu_work():
                nonlocal seq_counter, frame_count, _drop_count
                with gpu_lock:
                    # 2. Bridge
                    with Timer() as t_infer:
                        features_np = _run_bridge_step(bridge, chunk_tensor)
                        if cfg.DEVICE == "cuda": torch.cuda.synchronize()

                    # 3. Ditto Check & Run
                    with Timer() as t_ditto:
                        ditto.add_to_buffer(features_np)
                        frames = []
                        if ditto.is_ready():
                            frames = ditto.run_step()
                            if cfg.DEVICE == "cuda": torch.cuda.synchronize()

                return frames, t_infer.ms, t_ditto.ms

            try:
                frames, infer_ms, ditto_ms = await asyncio.to_thread(_gpu_work)
                ...
                if frame_count % 50 == 0:
                    profiler.print_summary(label=f"@ frame={frame_count}")

            except Exception as exc:
                logger.error(f"[pipeline_loop_task] GPU work error: {exc}")
                error_event.set()
        ...
        try:
            ...
            logger.exception(f"[pipeline_loop_task] Unhandled error: {exc}")
            error_event.set()
        finally:
            ditto.close()
            logger.info(f"[pipeline_loop_task] Done. Generated {frame_count} frames.")
    ...
    t_pipeline     = asyncio.create_task(pipeline_loop_task(),   name="pipeline")
    ...
    try:
        ...
        all_tasks = (t_ws_sender, t_pipeline, t_keepalive)
        try:
