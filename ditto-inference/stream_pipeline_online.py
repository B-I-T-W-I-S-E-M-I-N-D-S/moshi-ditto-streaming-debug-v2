...
class StreamSDK:
    ...
    def setup(self, source_path, output_path, **kwargs):
        ...
        self.start_threads = kwargs.get("start_threads", True)
    ...
    def setup_synchronous(self):
        self.item_buffer = np.zeros((0, self.wav2feat.feat_dim), dtype=np.float32)
        self.res_kp_seq = None
        self.res_kp_seq_valid_start = None if self.online_mode else 0
        self.global_idx = 0
        self.local_idx = 0
        self.gen_frame_idx = 0

    def add_to_buffer(self, item):
        self.item_buffer = np.concatenate([self.item_buffer, item], 0)
        if self.item_buffer.shape[0] >= self.audio2motion.valid_clip_len:
            self.audio_feat = np.concatenate([self.audio_feat, self.item_buffer], 0)
            self.item_buffer = np.zeros((0, self.wav2feat.feat_dim), dtype=np.float32)

    def is_ready(self):
        seq_frames = self.audio2motion.seq_frames
        aud_feat = self.audio_feat[self.local_idx: self.local_idx+seq_frames]
        if len(aud_feat) == 0:
            return False
        if len(aud_feat) < seq_frames:
            return False
        return True
    ...
    def run_step(self):
        ...
        aud_cond = self.condition_handler(aud_feat, self.global_idx + self.cond_idx_start)[None]

        # 1. audio2motion
        self.res_kp_seq = self.audio2motion(aud_cond, self.res_kp_seq)

        if self.res_kp_seq_valid_start is None:
            self.res_kp_seq_valid_start = self.res_kp_seq.shape[1] - self.audio2motion.fuse_length
            d0 = self.audio2motion.cvt_fmt(self.res_kp_seq[0:1])[0]
            self.motion_stitch.d0 = d0
            self.local_idx += real_valid_len
            self.global_idx += real_valid_len
            return []

        valid_res_kp_seq = self.res_kp_seq[:, self.res_kp_seq_valid_start: self.res_kp_seq_valid_start + real_valid_len]
        x_d_info_list = self.audio2motion.cvt_fmt(valid_res_kp_seq)

        res_frames = []
        for x_d_info in x_d_info_list:
            frame_idx = _mirror_index(self.gen_frame_idx, self.source_info_frames)
            ...
            res_frames.append(res_frame_rgb)
            self.gen_frame_idx += 1

        self.res_kp_seq_valid_start += real_valid_len
        self.local_idx += real_valid_len
        self.global_idx += real_valid_len

        L = self.res_kp_seq.shape[1]
        if L > seq_frames * 2:
            cut_L = L - seq_frames * 2
            self.res_kp_seq = self.res_kp_seq[:, cut_L:]
            self.res_kp_seq_valid_start -= cut_L
